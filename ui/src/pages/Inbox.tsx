...
function runFailureMessage(run: HeartbeatRun, t: TFunction): string {
  return firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? t("common:inbox.runExitedError");
}
...
export function formatJoinRequestInboxLabel(
  ...
  if (joinRequest.requestType !== "human") {
    if (t) {
      return joinRequest.agentName
        ? t("common:inbox.joinRequest.agentNamed", { name: joinRequest.agentName })
        : t("common:inbox.joinRequest.agent");
    }
  ...
  return t ? t("common:inbox.joinRequest.human") : "Human join request";
}
      ...
      <div className="flex items-start gap-2 sm:items-center">
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("common:inbox.markAsRead")}
              >
                <span className={cn(
              ...
              <button
                ...
                aria-label={t("common:inbox.dismissFromInbox")}
              >
        ...
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
          ...
          >
            ...
            {isRetrying ? t("common:inbox.retrying") : t("common:inbox.retry")}
          </Button>
          {!showUnreadSlot && (
            <button
              type="button"
              ...
              aria-label={t("common:inbox.dismiss")}
            >
              <X className="h-4 w-4" />
            </button>
          )}
...
export function Inbox() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("common:inbox.breadcrumb") }]);
  }, [setBreadcrumbs]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToApprove"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToReject"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToApproveJoin"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToRejectJoin"));
    },
  });
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToArchive"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("common:inbox.errors.failedToUndoArchive"));
    },
    onSuccess: (_data, id) => {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              {
                value: "mine",
                label: t("common:inbox.tabs.mine"),
              },
              {
                value: "recent",
                label: t("common:inbox.tabs.recent"),
              },
              { value: "unread", label: t("common:inbox.tabs.unread") },
              { value: "all", label: t("common:inbox.tabs.all") },
            ]}
          />
        </Tabs>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={nestingEnabled ? t("common:inbox.nestingDisable") : t("common:inbox.nestingEnable")}
          >
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-40 p-2">
              <div className="space-y-0.5">
                {([
                  ["none" as InboxWorkItemGroupBy, t("common:inbox.groupBy.none")],
                  ["type" as InboxWorkItemGroupBy, t("common:inbox.groupBy.type")],
                  ...(isolatedWorkspacesEnabled ? ([["workspace" as InboxWorkItemGroupBy, t("common:inbox.groupBy.workspace")]]) : []),
          ...
          <IssueColumnPicker
            ...
            title={t("common:inbox.columnPickerTitle")}
            iconOnly
          />
          {canMarkAllRead && (
            <>
              ...
              >
                {markAllReadMutation.isPending ? t("common:inbox.markingAllRead") : t("common:inbox.markAllRead")}
              </Button>
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("common:inbox.markAllReadDialog.title")}</DialogTitle>
                    <DialogDescription>
                      {t("common:inbox.markAllReadDialog.description", { count: unreadIssueIds.length })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("common:inbox.categoryPlaceholder")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("common:inbox.categories.allCategories")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("common:inbox.categories.myRecentIssues")}</SelectItem>
              <SelectItem value="join_requests">{t("common:inbox.categories.joinRequests")}</SelectItem>
              <SelectItem value="approvals">{t("common:inbox.categories.approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("common:inbox.categories.failedRuns")}</SelectItem>
              <SelectItem value="alerts">{t("common:inbox.categories.alerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("common:inbox.approvalStatusPlaceholder")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("common:inbox.approvalStatuses.all")}</SelectItem>
                <SelectItem value="actionable">{t("common:inbox.approvalStatuses.needsAction")}</SelectItem>
                <SelectItem value="resolved">{t("common:inbox.approvalStatuses.resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("common:inbox.empty.noSearchResults")
              : tab === "mine"
              ? t("common:inbox.empty.inboxZero")
              : tab === "unread"
              ? t("common:inbox.empty.noNew")
              : tab === "recent"
                ? t("common:inbox.empty.noRecent")
                : t("common:inbox.empty.noFilterResults")
          }
        />
      )}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("common:inbox.sections.archived") : t("common:inbox.sections.otherResults")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
