...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("settings:company.import.previewEmpty")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("settings:company.import.binaryPreview")}
          </div>
        )}
      </div>
    </div>
      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">
            {t("settings:company.import.renames.title")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {t("settings:company.import.renames.items", { count: conflicts.length })}
          </span>
        </div>
          ...
          {conflicts.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isSkipped ? t("settings:company.import.renames.skipped") : t("settings:company.import.renames.skip")}
                </button>
                  ...
                  >
                    {isConfirmed ? (
                      <>
                        ...
                        {t("settings:company.import.renames.confirmed")}
                      </>
                    ) : (
                      t("settings:company.import.renames.confirmRename")
                    )}
                  </button>
                )}
              </div>
...
function AdapterPickerList({
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          <h3 className="text-sm font-medium">{t("settings:company.import.adapters.title")}</h3>
          <span className="text-xs text-muted-foreground">
            {t("settings:company.import.adapters.agents", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
              ...
              <div key={agent.slug}>
                <div className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  ...
                  >
                    ...
                    {t("settings:company.import.adapters.configureButton")}
                  </button>
                </div>
                {isExpanded && (
...
export function CompanyImport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings:company.import.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company.import.toasts.previewFailed"),
        body: err instanceof Error ? err.message : t("settings:company.import.toasts.previewFailedBody"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("settings:company.import.toasts.importComplete"),
        body: t("settings:company.import.toasts.importCompleteBody", { company: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company.import.toasts.importFailed"),
        body: err instanceof Error ? err.message : t("settings:company.import.toasts.importFailedBody"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("settings:company.import.toasts.packageReadFailed"),
        body: err instanceof Error ? err.message : t("settings:company.import.toasts.packageReadFailedBody"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("settings:company.import.noCompany")} />;
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("settings:company.import.sourceTitle")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("settings:company.import.sourceDescription")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("settings:company.import.github") },
              { key: "local", icon: Upload, label: t("settings:company.import.localZip") },
            ] as const
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t("settings:company.import.chooseZip")}
              </Button>
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("settings:company.import.localZipFiles", {
            ...
            {!localPackage && (
              <p className="mt-2 text-xs text-muted-foreground">
                {t("settings:company.import.localZipHelp")}
              </p>
            )}
          </div>
        ) : (
          <Field
            label={t("settings:company.import.githubUrl.label")}
            hint={t("settings:company.import.githubUrl.hint")}
          >
            <input
        ...
        <Field label={t("settings:company.import.target.label")} hint={t("settings:company.import.target.hint")}>
          ...
          >
            <option value="new">{t("settings:company.import.target.newCompany")}</option>
            <option value="existing">
              {t("settings:company.import.target.existingCompany", { name: selectedCompany?.name })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            label={t("settings:company.import.newCompanyName.label")}
            hint={t("settings:company.import.newCompanyName.hint")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("settings:company.import.newCompanyName.placeholder")}
            />
          </Field>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("settings:company.import.preview.previewing") : t("settings:company.import.preview.button")}
          </Button>
        </div>
      </div>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium">
                {t("settings:company.import.preview.title")}
              </span>
              <span className="text-muted-foreground">
                {t("settings:company.import.preview.filesSelected", { selected: selectedCount, total: totalFiles, count: totalFiles })}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("settings:company.import.preview.conflicts", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("settings:company.import.preview.errors", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("settings:company.import.button", { count: selectedCount })}
            </Button>
          </div>
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">{t("settings:company.import.packageFiles")}</h2>
              </div>
