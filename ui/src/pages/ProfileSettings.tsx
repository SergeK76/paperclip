...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings:instance.breadcrumb") },
      { label: t("settings:profile.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("settings:profile.displayName.placeholder");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.errors.updateFailed"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings:profile.errors.selectCompanyFirst"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.errors.uploadFailed"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.errors.removeFailed"));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("settings:profile.loadFailed")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("settings:profile.displayName.placeholder");
  ...
  const uploadHint = selectedCompany
    ? t("settings:profile.uploadHintWithCompany", { company: selectedCompany.name })
    : t("settings:profile.uploadHintNoCompany");
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings:profile.uploadHintPrefix")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("settings:profile.displayName.label")}</Label>
            <Input
              id="profile-name"
              value={name}
