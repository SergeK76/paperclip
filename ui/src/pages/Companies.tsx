...
export function Companies() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("common:companies.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-end">
        <Button size="sm" onClick={() => openOnboarding()}>
          ...
          {t("common:companies.newCompany")}
        </Button>
      </div>

      <div className="h-6">
        {loading && <p className="text-sm text-muted-foreground">{t("common:companies.loading")}</p>}
            ...
            >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      >
                        ...
                        {t("common:companies.deleteCompany")}
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ...
              <div className="flex items-center gap-3 sm:gap-5 mt-4 text-sm text-muted-foreground flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{t("common:companies.agentCount", { count: agentCount })}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <CircleDot className="h-3.5 w-3.5" />
                  <span>{t("common:companies.issueCount", { count: issueCount })}</span>
                </div>
                <div className="flex items-center gap-1.5 tabular-nums">
                  ...
                  <span>
                    ...
                    {company.budgetMonthlyCents > 0
                      ...
                      : <span className="text-xs ml-1">{t("common:companies.unlimitedBudget")}</span>}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 ml-auto">
                  <Calendar className="h-3.5 w-3.5" />
                  <span>{t("common:companies.created", { time: relativeTime(company.createdAt) })}</span>
                </div>
              </div>
              ...
              {isConfirmingDelete && (
                ...
                >
                  <p className="text-sm text-destructive font-medium">
                    {t("common:companies.deleteConfirm")}
                  </p>
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    ...
                    <Button
                      ...
                      disabled={deleteMutation.isPending}
                    >
                      {deleteMutation.isPending ? t("common:companies.deleting") : t("common:companies.delete")}
                    </Button>
                  </div>
                </div>
              )}
