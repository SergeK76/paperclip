  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("costs:financeCard.title")}</CardTitle>
        <CardDescription>
          {t("costs:financeCard.description")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("costs:financeCard.debits")}
          value={formatCents(debitCents)}
          subtitle={t("costs:financeCard.debitsSubtitle", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("costs:financeCard.credits")}
          value={formatCents(creditCents)}
          subtitle={t("costs:financeCard.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs:financeCard.net")}
          value={formatCents(netCents)}
          subtitle={t("costs:financeCard.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs:financeCard.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs:financeCard.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <MetricTile
              ...
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ...
                  : t("costs:metrics.openValue")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs:metrics.budgetIncidentSubtitle", { agentCount: budgetData?.pausedAgentCount ?? 0, projectCount: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs:metrics.budgetUtilSubtitle", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("costs:metrics.noCapSubtitle")
              }
              icon={Coins}
            />
            <MetricTile
            ...
            <MetricTile
              ...
              subtitle={t("costs:metrics.financeEventsSubtitle", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs:customDatePrompt")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs:overview.inferenceLedger")}</CardTitle>
                    <CardDescription>
                      {t("costs:overview.inferenceLedgerDesc")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs:overview.budgetLabel", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costs:overview.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("costs:overview.usageLabel")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs:overview.utilizationText", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("costs:overview.byAgent")}</CardTitle>
                    <CardDescription>{t("costs:overview.byAgentDesc")}</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs:overview.noAgentEvents")}</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs:overview.byProject")}</CardTitle>
                      <CardDescription>{t("costs:overview.byProjectDesc")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs:overview.noProjectEvents")}</p>
                      ) : (
                        spendData?.byProject.map((row, index) => (
                          ...
                          >
                            <span className="truncate">{row.projectName ?? row.projectId ?? t("costs:overview.unattributed")}</span>
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs:overview.financeTimelineEmpty")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("costs:budgets.controlPlane")}</CardTitle>
                  <CardDescription>
                    {t("costs:budgets.controlPlaneDesc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("costs:budgets.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("costs:budgets.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("costs:budgets.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("costs:budgets.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("costs:budgets.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("costs:budgets.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("costs:budgets.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("costs:budgets.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs:budgets.activeIncidentsTitle")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs:budgets.activeIncidentsDesc")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {scopeType === "company"
                            ? t("costs:budgets.scopeCompanyTitle")
                            : scopeType === "agent"
                              ? t("costs:budgets.scopeAgentTitle")
                              : t("costs:budgets.scopeProjectTitle")}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs:budgets.scopeCompanyDesc")
                            : scopeType === "agent"
                              ? t("costs:budgets.scopeAgentDesc")
                              : t("costs:budgets.scopeProjectDesc")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs:noProviderEvents")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs:noBillerEvents")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("costs:financeTab.byBiller")}</CardTitle>
                      <CardDescription>{t("costs:financeTab.byBillerDesc")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs:financeTab.noFinanceEvents")}</p>
                      ) : (
