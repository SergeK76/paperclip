...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:instance.heartbeats.errors.updateFailed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:instance.heartbeats.errors.disableAllFailed"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings:instance.heartbeats.loadFailed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("settings:instance.heartbeats.description")}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("settings:instance.heartbeats.disabled_stat")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("settings:instance.heartbeats.companies", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("settings:instance.heartbeats.confirmDisableAll", { count: enabledCount }))) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("settings:instance.heartbeats.empty")}
        />
      ) : (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("settings:instance.heartbeats.fullConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("settings:instance.heartbeats.disableHeartbeat") : t("settings:instance.heartbeats.enableHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
