...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("settings:company.export.previewEmpty")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("settings:company.export.binaryPreview")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings:company.export.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company.export.toasts.failed"),
        body: err instanceof Error ? err.message : t("settings:company.export.toasts.loadFailed"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("settings:company.export.toasts.downloadSuccess"),
        body: t("settings:company.export.toasts.downloadBody", { count: resultCheckedFiles.size, rootPath: result.rootPath }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("settings:company.export.toasts.failed"),
        body: err instanceof Error ? err.message : t("settings:company.export.toasts.buildFailed"),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("settings:company.export.noCompany")} />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message={t("settings:company.export.loadingData")} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {t("settings:company.export.title", { company: selectedCompany?.name ?? "Company" })}
            </span>
            <span className="text-muted-foreground">
              {t("settings:company.export.filesSelected", { selected: selectedCount, total: totalFiles, count: totalFiles })}
            </span>
            {warnings.length > 0 && (
          ...
          >
            ...
            {downloadMutation.isPending
              ...
              : t("settings:company.export.button", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
        <aside className="flex flex-col border-r border-border overflow-hidden">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">{t("settings:company.export.packageFiles")}</h2>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("settings:company.export.searchFiles")}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("settings:company.export.showMoreIssues", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
