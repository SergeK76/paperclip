...
function ActorIdentity({ evt, agentMap, userProfileMap }: { evt: ActivityEvent; agentMap: Map<string, Agent>; userProfileMap?: Map<string, import("../lib/company-members").CompanyUserProfile> }) {
  ...
  if (evt.actorType === "system") return <Identity name={t("issues:detail.actor.system")} size="sm" />;
  if (evt.actorType === "user") {
    const profile = userProfileMap?.get(id);
    return <Identity name={profile?.label ?? t("issues:detail.actor.board")} avatarUrl={profile?.image} size="sm" />;
  }
  return <Identity name={id || t("issues:detail.actor.unknown")} size="sm" />;
}
  ...
  return (
    <div className="flex items-center w-full">
      <Button
        ...
        aria-label={t("issues:detail.toolbar.backToInbox")}
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {issueIdProp && !issueHidden && (
          <Button
            ...
            aria-label={t("issues:detail.toolbar.archiveFromInbox")}
          >
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("issues:detail.toolbar.moreActions")}>
          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            >
              ...
              {t("issues:detail.toolbar.copyMarkdown")}
            </button>
            <button
              className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            ...
            >
              ...
              {t("issues:detail.toolbar.properties")}
            </button>
            {issueIdProp && (
              <button
              ...
              >
                ...
                {t("issues:detail.toolbar.hideIssue")}
              </button>
            )}
          </PopoverContent>
        </Popover>
    ...
    <div className="space-y-3">
      {hasOlderComments ? (
        <div className="flex justify-center">
          ...
          >
            {commentsLoadingOlder ? t("issues:detail.chat.loadingEarlier") : t("issues:detail.chat.loadEarlier")}
          </Button>
        </div>
      ) : null}
      <IssueChatThread
    ...
    <>
      ...
      {linkedRuns && linkedRuns.length > 0 && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          <div className="text-sm font-medium text-muted-foreground mb-1">{t("issues:detail.activity.costSummary")}</div>
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens ? (
            <div className="text-xs text-muted-foreground">{t("issues:detail.activity.noCostData")}</div>
          ) : (
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground tabular-nums">
              ...
              {issueCostSummary.hasTokens && (
                <span>
                  {t("issues:detail.activity.tokens")} {formatTokens(issueCostSummary.totalTokens)}
                  {issueCostSummary.cached > 0
                    ? ` (${t("issues:detail.activity.tokenIn")} ${formatTokens(issueCostSummary.input)}, ${t("issues:detail.activity.tokenOut")} ${formatTokens(issueCostSummary.output)}, ${t("issues:detail.activity.tokenCached")} ${formatTokens(issueCostSummary.cached)})`
                    : ` (${t("issues:detail.activity.tokenIn")} ${formatTokens(issueCostSummary.input)}, ${t("issues:detail.activity.tokenOut")} ${formatTokens(issueCostSummary.output)})`}
                </span>
              )}
            </div>
          )}
      ...
      {!activity || activity.length === 0 ? (
        <p className="text-xs text-muted-foreground">{t("issues:detail.activity.noActivity")}</p>
      ) : (
...
export function IssueDetail() {
  ...
  const sourceBreadcrumb = useMemo(
    () => readIssueDetailBreadcrumb(issueId, location.state, location.search) ?? { label: t("issues:detail.breadcrumbSource"), href: "/issues" },
  ...
  const breadcrumbTitle = issue?.title ?? issueId ?? t("issues:detail.breadcrumbFallback");
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issues:detail.toasts.updateFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.updateFailedBody"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issues:detail.toasts.updateFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.subUpdateFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const approvalDecision = useMutation({
    ...
    onSuccess: (_approval, variables) => {
      ...
      pushToast({
        title: variables.action === "approve" ? t("issues:detail.toasts.approvalApproved") : t("issues:detail.toasts.approvalRejected"),
        tone: "success",
      });
    },
    onError: (err, variables) => {
      pushToast({
        title: variables.action === "approve" ? t("issues:detail.toasts.approvalFailed") : t("issues:detail.toasts.rejectionFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.approvalFailedBody"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const addComment = useMutation({
    ...
    onSuccess: async (comment, _variables, context) => {
      ...
      if (context?.optimisticCommentId && cancelledQueuedOptimisticCommentIdsRef.current.has(context.optimisticCommentId)) {
        ...
        try {
          ...
          pushToast({
            title: t("issues:detail.toasts.cancelFailed"),
            body: err instanceof Error ? err.message : t("issues:detail.toasts.cancelFailedBody"),
            tone: "error",
          });
        }
      }
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issues:detail.toasts.commentFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.commentFailedBody"),
        tone: "error",
      });
    },
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueRunState();
      pushToast({
        title: t("issues:detail.toasts.interruptRequested"),
        body: t("issues:detail.toasts.interruptRequestedBody"),
        tone: "success",
      });
    },
    onError: (err, _runId, context) => {
      ...
      pushToast({
        title: t("issues:detail.toasts.interruptFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.interruptFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const cancelQueuedComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issues:detail.toasts.queueCanceled"),
        body: t("issues:detail.toasts.queueCanceledBody"),
        tone: "success",
      });
    },
    onError: (err) => {
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("issues:detail.toasts.feedbackSavedShare")
              : t("issues:detail.toasts.feedbackSavedLocal")
            : variables.allowSharing
              ? t("issues:detail.toasts.feedbackSavedSharing")
              : t("issues:detail.toasts.feedbackSaved"),
        tone: "success",
      });
    },
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issues:detail.toasts.feedbackFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.feedbackFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const uploadAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issues:detail.toasts.uploadFailed"));
    },
  });
  ...
  const importMarkdownDocument = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issues:detail.toasts.importFailed"));
    },
  });
  ...
  const deleteAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issues:detail.toasts.deleteFailed"));
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("issues:detail.toasts.archived"), tone: "success" });
    },
    onError: (err) => {
      pushToast({
        title: t("issues:detail.toasts.archiveFailed"),
        body: err instanceof Error ? err.message : t("issues:detail.toasts.archiveFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("issues:detail.copiedToClipboard"), tone: "success" });
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("issues:detail.uploading") : (
          <>
            ...
            <span className="sm:hidden">{t("issues:detail.uploadShort")}</span>
          </>
        )}
      </Button>
    </>
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={t("issues:detail.copyMarkdown")}
              >
              ...
              <Button
                ...
                title={t("issues:detail.propertiesTitle")}
              >
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            <Button
              ...
              size="icon-xs"
              onClick={copyIssueToClipboard}
              title={t("issues:detail.actions.copyAsMarkdown")}
            >
              {copied ? <Check className="h-4 w-4 text-green-500" /> : <Copy className="h-4 w-4" />}
            </Button>
            <Button
      ...
      {showRichSubIssuesSection ? (
        <div className="space-y-3">
          ...
          <IssuesList
            ...
            createIssueLabel={t("issues:detail.subIssueLabel")}
            ...
            {imageAttachments.map((attachment) => (
              ...
              >
                ...
                {confirmDeleteId === attachment.id ? (
                  ...
                  >
                    <p className="text-xs text-white font-medium">{t("issues:detail.deleteConfirm")}</p>
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="chat" className="gap-1.5">
            ...
            {t("issues:detail.tabs.chat")}
          </TabsTrigger>
          <TabsTrigger value="activity" className="gap-1.5">
            <ActivityIcon className="h-3.5 w-3.5" />
            {t("issues:detail.tabs.activity")}
          </TabsTrigger>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-[85dvh] pb-[env(safe-area-inset-bottom)]">
          <SheetHeader>
            <SheetTitle className="text-sm">{t("issues:detail.propertiesTitle")}</SheetTitle>
          </SheetHeader>
