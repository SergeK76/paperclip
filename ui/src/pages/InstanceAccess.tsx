...
export function InstanceAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings:instance.breadcrumb"), href: "/instance/settings/general" },
      { label: t("settings:instance.access.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateCompanyAccessMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("settings:instance.access.toasts.companyUpdated"), tone: "success" });
    },
  });
  ...
  const setAdminMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("settings:instance.access.toasts.instanceRoleUpdated"), tone: "success" });
    },
  });

  if (usersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings:instance.access.loading")}</div>;
  }

  if (usersQuery.error) {
    const message =
      usersQuery.error instanceof ApiError && usersQuery.error.status === 403
        ? t("settings:instance.access.adminRequired")
        : usersQuery.error instanceof Error
          ? usersQuery.error.message
          : t("settings:instance.access.loadFailed");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings:instance.access.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("settings:instance.access.description")}
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className="space-y-4 rounded-xl border border-border bg-card p-4">
          <label className="block space-y-2 text-sm">
            <span className="font-medium">{t("settings:instance.access.search.label")}</span>
            <input
              className="w-full rounded-md border border-border bg-background px-3 py-2"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder={t("settings:instance.access.search.placeholder")}
            />
          </label>
            ...
            {(usersQuery.data ?? []).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("settings:instance.access.memberships", { count: user.activeCompanyMembershipCount })}
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          {!selectedUserId ? (
            <div className="text-sm text-muted-foreground">{t("settings:instance.access.selectUser")}</div>
          ) : userAccessQuery.isLoading ? (
            <div className="text-sm text-muted-foreground">{t("settings:instance.access.loadingUser")}</div>
          ) : userAccessQuery.error ? (
            <div className="text-sm text-destructive">
              {userAccessQuery.error instanceof Error ? userAccessQuery.error.message : t("settings:instance.access.loadUserFailed")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                >
                  {selectedUser?.isInstanceAdmin ? t("settings:instance.access.companyAccess.removeAdmin") : t("settings:instance.access.companyAccess.promoteAdmin")}
                </Button>
              </div>

              <div className="space-y-3">
                <div>
                  <h2 className="text-sm font-semibold">{t("settings:instance.access.companyAccess.title")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("settings:instance.access.companyAccess.description")}
                  </p>
                </div>
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {updateCompanyAccessMutation.isPending ? t("settings:instance.access.companyAccess.saving") : t("settings:instance.access.companyAccess.save")}
                  </Button>
                </div>
              </div>

              <div className="space-y-2">
                <h2 className="text-sm font-semibold">{t("settings:instance.access.currentMemberships.title")}</h2>
                <div className="space-y-2">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground">
                          {membership.membershipRole || t("settings:instance.access.currentMemberships.unset")} • {membership.status}
                        </div>
                      </div>
