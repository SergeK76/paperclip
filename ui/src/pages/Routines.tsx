      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines:row.statusArchived") : isDraft ? t("routines:row.statusDraft") : t("routines:row.statusPaused")}
            </span>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-2">
            ...
            />
            <span>{routine.projectId ? (project?.name ?? t("routines:row.unknownProject")) : t("routines:row.noProject")}</span>
          </span>
          <span className="flex items-center gap-2">
            {agent?.icon ? <AgentIcon icon={agent.icon} className="h-3.5 w-3.5 shrink-0" /> : null}
            <span>{routine.assigneeAgentId ? (agent?.name ?? t("routines:row.unknownAgent")) : t("routines:row.noAgent")}</span>
          </span>
          <span>
            {formatLastRunTimestamp(routine.lastRun?.triggeredAt, t("routines:row.neverRun"))}
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("routines:row.toggleDisable", { title: routine.title }) : t("routines:row.toggleEnable", { title: routine.title })}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines:row.labelArchived") : isDraft ? t("routines:row.labelDraft") : enabled ? t("routines:row.labelOn") : t("routines:row.labelOff")}
          </span>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("routines:row.moreActions", { title: routine.title })}>
              <MoreHorizontal className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link to={href}>{t("routines:row.edit")}</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={runningRoutineId === routine.id || isArchived}
              onClick={() => onRunNow(routine)}
            >
              {runningRoutineId === routine.id ? t("routines:row.running") : t("routines:row.runNow")}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              ...
              disabled={isStatusPending || isArchived}
            >
              {enabled ? t("routines:row.pause") : t("routines:row.enable")}
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => onToggleArchived(routine)}
              disabled={isStatusPending}
            >
              {routine.status === "archived" ? t("routines:row.restore") : t("routines:row.archive")}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
...
export function Routines() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("routines:list.breadcrumb") }]);
  }, [setBreadcrumbs, t]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines:toasts.created"),
        body: routine.assigneeAgentId
          ? t("routines:toasts.createdBodyWithAgent")
          : t("routines:toasts.createdBodyDraft"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines:toasts.updateFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines:toasts.runFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const concurrencyPolicyDescriptions = useMemo<Record<string, string>>(() => ({
    coalesce_if_active: t("routines:concurrencyPolicies.coalesce_if_active"),
    always_enqueue: t("routines:concurrencyPolicies.always_enqueue"),
    skip_if_active: t("routines:concurrencyPolicies.skip_if_active"),
  }), [t]);
  const catchUpPolicyDescriptions = useMemo<Record<string, string>>(() => ({
    skip_missed: t("routines:catchUpPolicies.skip_missed"),
    enqueue_missed_with_cap: t("routines:catchUpPolicies.enqueue_missed_with_cap"),
  }), [t]);
  const routineGroups = useMemo(
    () => buildRoutineGroups(routines ?? [], routineViewState.groupBy, projectById, agentById, {
      noProject: t("routines:list.groupLabels.noProject"),
      unknownProject: t("routines:list.groupLabels.unknownProject"),
      unassigned: t("routines:list.groupLabels.unassigned"),
      unknownAgent: t("routines:list.groupLabels.unknownAgent"),
    }),
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">
            {t("routines:list.title")}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("routines:list.description")}
          </p>
        </div>
        <Button onClick={() => setComposerOpen(true)}>
          <Plus className="mr-2 h-4 w-4" />
          {t("routines:list.createButton")}
        </Button>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines:list.tabs.routines") },
            { value: "runs", label: t("routines:list.tabs.runs") },
          ]}
        />
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routines:list.count", { count: (routines ?? []).length })}
            </p>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                  <span className="hidden sm:inline">{t("routines:list.groupButton")}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["project", t("routines:list.groupBy.project")],
                    ["assignee", t("routines:list.groupBy.agent")],
                    ["none", t("routines:list.groupBy.none")],
                  ] as [RoutineGroupBy, string][]).map(([value, label]) => (
                    <button
                      key={value}
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines:composer.heading")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines:composer.description")}
              </p>
            </div>
            <Button
              variant="ghost"
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines:composer.advanced.title")}</p>
                    <p className="text-sm text-muted-foreground">{t("routines:composer.advanced.description")}</p>
                  </div>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines:composer.creating") : t("routines:composer.create")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines:composer.failedToCreate")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines:list.failedToLoad")}
          </CardContent>
        </Card>
      ) : null}
        ...
        <div>
          {(routines ?? []).length === 0 ? (
            <div className="py-12">
              <EmptyState
                icon={Repeat}
                message={t("routines:list.noRoutines")}
              />
            </div>
          ) : (
