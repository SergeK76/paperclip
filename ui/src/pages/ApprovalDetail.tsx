...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("approvals:list.breadcrumb"), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("approvals:detail.fallback") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.rejectFailed")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.revisionFailed")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.resubmitFailed")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.commentFailed")),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approvals:detail.errors.deleteFailed")),
  });
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("approvals:detail.cta.reviewLinkedIssues")
              : t("approvals:detail.cta.reviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("approvals:detail.cta.openHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("approvals:detail.cta.backToApprovals"),
            to: "/approvals",
          };
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvals:detail.approvedBanner.title")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvals:detail.approvedBanner.description")}
                </p>
              </div>
            </div>
            <Button
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("approvals:detail.budgetStop")} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approvals:detail.deleteDisapprovedConfirm"))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("approvals:detail.deleteDisapproved")}
            </Button>
          )}
        </div>
      </div>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("approvals:detail.commentBoard")} size="sm" />
                )}
