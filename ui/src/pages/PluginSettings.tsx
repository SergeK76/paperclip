...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("plugins:breadcrumb.settings"), href: "/instance/settings/heartbeats" },
      { label: t("plugins:breadcrumb.plugins"), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("plugins:breadcrumb.pluginDetails") },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("plugins:settings.loading")}</div>;
  }
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          align="start"
          items={[
            { value: "configuration", label: t("plugins:settings.tabs.configuration") },
            { value: "status", label: t("plugins:settings.tabs.status") },
          ]}
          value={activeTab}
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">{t("plugins:settings.about.title")}</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("plugins:settings.about.descriptionLabel")}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("plugins:settings.about.authorLabel")}</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">{t("plugins:settings.about.categoriesLabel")}</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("plugins:settings.about.noneCategories")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("plugins:settings.settingsTitle")}</h2>
              </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins:status.runtimeDashboard.title")}
                  </CardTitle>
                  <CardDescription>
                    {t("plugins:status.runtimeDashboard.description")}
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins:status.worker.title")}
                        </h3>
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins:status.worker.statusLabel")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins:status.worker.pidLabel")}</span>
                              <span className="font-mono text-xs">{dashboardData.worker.pid ?? "—"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins:status.worker.uptimeLabel")}</span>
                              <span className="text-xs">{formatUptime(dashboardData.worker.uptime)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("plugins:status.worker.pendingRpcsLabel")}</span>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    ...
                                    {t("plugins:status.worker.crashesLabel")}
                                  </span>
                                  <span className="text-xs">
                                    {dashboardData.worker.consecutiveCrashes} {t("plugins:status.worker.consecutive")} / {dashboardData.worker.totalCrashes} {t("plugins:status.worker.total")}
                                  </span>
                                </div>
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("plugins:status.worker.lastCrashLabel")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins:status.worker.noWorker")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins:status.jobs.empty")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-medium mb-3 flex items-center gap-1.5">
                          ...
                          {t("plugins:status.webhooks.title")}
                        </h3>
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("plugins:status.webhooks.empty")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("plugins:status.lastChecked")}{new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("plugins:status.noDashboard")}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-1.5">
                      ...
                      {t("plugins:status.logs.title")}
                    </CardTitle>
                    <CardDescription>{t("plugins:status.logs.description", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    <ActivitySquare className="h-4 w-4" />
                    {t("plugins:status.health.title")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("plugins:status.health.checking")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">{t("plugins:status.health.overallLabel")}</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      <div className="flex items-center justify-between">
                        <span>{t("plugins:status.lifecycle")}</span>
                        <Badge variant={statusVariant}>{displayStatus}</Badge>
                      </div>
                      <p>{t("plugins:status.health.waitingHint")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{t("plugins:status.details.title")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins:status.details.pluginId")}</span>
                    <span className="font-mono text-xs text-right">{plugin.id}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins:status.details.pluginKey")}</span>
                    <span className="font-mono text-xs text-right">{plugin.pluginKey}</span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins:status.details.npmPackage")}</span>
                    <span className="max-w-[170px] truncate text-right text-xs" title={plugin.packageName}>
                      {plugin.packageName}
                    </span>
                  </div>
                  <div className="flex justify-between gap-3">
                    <span>{t("plugins:status.details.version")}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("plugins:status.permissions.title")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("plugins:status.permissions.none")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("plugins:configForm.saved") });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("plugins:configForm.saveFailed") });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("plugins:configForm.testPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("plugins:configForm.testFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("plugins:configForm.testFailed") });
    },
  });
        ...
        >
          {saveMutation.isPending ? (
            <>
              ...
              {t("plugins:configForm.saving")}
            </>
          ) : (
            t("plugins:configForm.save")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              <>
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                {t("plugins:configForm.testing")}
              </>
            ) : (
              t("plugins:configForm.test")
            )}
          </Button>
        )}
      </div>
