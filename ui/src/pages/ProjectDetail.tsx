  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-muted-foreground">{t("projects:detail.overview.status")}</span>
          <div className="mt-1">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("projects:detail.overview.targetDate")}</span>
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-label={t("projects:detail.colorPicker.changeColor")}
      />
      {open && (
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {PROJECT_COLORS.map((color) => (
              <button
                ...
                aria-label={t("projects:detail.colorPicker.selectColor", { color })}
              />
            ))}
          </div>
        </div>
...
function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("projects:detail.noWorkspaceActivity")}</p>;
  }
        ...
        {cleanupFailedSummaries.length > 0 ? (
          <div className="space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("projects:detail.cleanupNeeded")}
            </div>
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("projects:list.fallback");
      if (archived) {
    ...
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("projects:detail.failedToArchive") : t("projects:detail.failedToUnarchive"),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("projects:list.fallback") },
    ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("projects:detail.tabs.issues") },
