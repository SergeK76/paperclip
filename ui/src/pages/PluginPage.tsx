  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("plugins:page.selectCompany")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("plugins:page.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("plugins:page.routeConflictPre")} <code>{pluginRoutePath}</code>{t("plugins:page.routeConflictPost")}
        </div>
      );
    }
  }
