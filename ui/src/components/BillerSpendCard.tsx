    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t("costs:billerSpend.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t("costs:billerSpend.tokensOut")}
              {" · "}
              {t("costs:billerSpend.providers", { count: row.providerCount })}
              {" · "}
              {t("costs:billerSpend.models", { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            label={t("costs:billerSpend.periodSpend")}
            percentUsed={budgetPct}
            leftLabel={formatCents(row.costCents)}
            rightLabel={t("costs:billerSpend.ofAllocation", { pct: Math.round(budgetPct) })}
          />
        )}

        <div className="text-xs text-muted-foreground">
          {t("costs:billerSpend.meteredRuns", { count: row.apiRunCount })}
          {" · "}
          {t("costs:billerSpend.subscriptionRuns", { count: row.subscriptionRunCount })}
          {" · "}
          {t("costs:billerSpend.thisWeek", { amount: formatCents(weekSpendCents) })}
        </div>
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costs:billerSpend.billingTypes")}
              </p>
        ...
        {providerBreakdown.length > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("costs:billerSpend.upstreamProviders")}
              </p>
              <div className="space-y-1.5">
