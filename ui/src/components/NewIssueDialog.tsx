...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("newIssue.uploadWarningTitle", { ref: issueRef }),
          body: t("newIssue.uploadWarningBody", { count: failures.length }),
          tone: "warn",
          action: prefix
            ? { label: t("newIssue.openIssue", { ref: issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssue.createErrorFallback");
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("newIssue.newSubIssue") : t("newIssue.newIssue")}</span>
          </div>
              ...
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssue.assigneePlaceholder")}</span>
                  )
                }
                renderOption={(option) => {
              ...
              <InlineEntitySelector
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssue.projectPlaceholder")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={t("newIssue.addParticipantTitle")}
                  >
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              ...
              >
                {EXECUTION_WORKSPACE_MODES.map((option) => (
                  <option key={option.value} value={option.value}>
                    {t(`newIssue.workspaceMode.${option.value}`)}
                  </option>
                ))}
              </select>
              {executionWorkspaceMode === "reuse_existing" && (
                ...
                >
                  <option value="">{t("newIssue.chooseWorkspace")}</option>
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssue.reusingWorkspace", { name: selectedReusableExecutionWorkspace.name, location: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("newIssue.existingWorkspace") })}
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {parentExecutionWorkspaceLabel
                    ? t("newIssue.parentWorkspaceWarningWithLabel", { label: parentExecutionWorkspaceLabel })
                    : t("newIssue.parentWorkspaceWarning")}
                </div>
              ) : null}
            </div>
            </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {thinkingEffortOptions.map((option) => (
                      ...
                      >
                        {t(`newIssue.thinkingOption.${option.value || "default"}`)}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {t(`newIssue.status.${currentStatus.value}`)}
              </button>
            </PopoverTrigger>
              ...
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                {currentPriority ? (
                  <>
                    ...
                    {t(`newIssue.priority.${currentPriority.value}`)}
                  </>
                ) : (
                  <>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("newIssue.creating") : isSubIssueMode ? t("newIssue.createSubIssue") : t("newIssue.createIssueBtn")}</span>
              </span>
            </Button>
          </div>
        </div>
