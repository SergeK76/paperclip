...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: TFunction) {
  return windowKind === "lifetime" ? t("costs:budgetPolicy.lifetimeBudget") : t("costs:budgetPolicy.monthlyUtcBudget");
}
...
export function BudgetPolicyCard({
  ...
  const softAlertStr = summary.paused && summary.pauseReason
    ? t("costs:budgetPolicy.softAlertWithPause", { warnPercent: summary.warnPercent, reason: summary.pauseReason })
    : t("costs:budgetPolicy.softAlert", { warnPercent: summary.warnPercent });

  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs:budgetPolicy.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("costs:budgetPolicy.percentOfLimit", { pct: summary.utilizationPercent }) : t("costs:budgetPolicy.noCap")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("costs:budgetPolicy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("costs:budgetPolicy.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">{softAlertStr}</div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("costs:budgetPolicy.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("costs:budgetPolicy.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("costs:budgetPolicy.pausedProject")
          : t("costs:budgetPolicy.pausedScope")}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("costs:budgetPolicy.budgetLabel")}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("costs:budgetPolicy.saving") : summary.amount > 0 ? t("costs:budgetPolicy.updateBudget") : t("costs:budgetPolicy.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("costs:budgetPolicy.statusPaused") : summary.status === "warning" ? t("costs:budgetPolicy.statusWarning") : summary.status === "hard_stop" ? t("costs:budgetPolicy.statusHardStop") : t("costs:budgetPolicy.statusHealthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("costs:budgetPolicy.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
