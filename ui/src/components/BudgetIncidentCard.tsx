        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
              {incident.scopeType} {t("costs:budgetIncident.hardStop")}
            </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              {t("costs:budgetIncident.spendingReached", { amount: formatCents(incident.amountObserved), limit: formatCents(incident.amountLimit) })}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("costs:budgetIncident.pausedProject")
              : t("costs:budgetIncident.pausedScope")}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("costs:budgetIncident.newBudget")}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("costs:budgetIncident.applying") : t("costs:budgetIncident.raiseAndResume")}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("costs:budgetIncident.mustExceedSpend")}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>
            {t("costs:budgetIncident.keepPaused")}
          </Button>
        </div>
      </CardContent>
    </Card>
