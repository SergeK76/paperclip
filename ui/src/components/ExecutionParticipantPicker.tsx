...
export function ExecutionParticipantPicker({
  ...
  const participantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) return userLabel(value.slice("user:".length)) ?? t("workspaces:participantPicker.user");
    return value;
  };
  ...
  const label = stageType === "review" ? t("workspaces:participantPicker.reviewers") : t("workspaces:participantPicker.approvers");
    ...
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setSearch(""); }}>
      ...
      <PopoverContent className="p-1 w-56" align="start" collisionPadding={16}>
        <input
          ...
          placeholder={stageType === "review" ? t("workspaces:participantPicker.searchReviewers") : t("workspaces:participantPicker.searchApprovers")}
          value={search}
        ...
        <div className="max-h-48 overflow-y-auto overscroll-contain">
          ...
          >
            {stageType === "review" ? t("workspaces:participantPicker.noReviewers") : t("workspaces:participantPicker.noApprovers")}
          </button>
          {currentUserId && (
            <button
              className={cn(
            ...
            >
              ...
              {t("workspaces:participantPicker.assignToMe")}
            </button>
          )}
          {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
            ...
            >
              <User className="h-3 w-3 shrink-0 text-muted-foreground" />
              {creatorUserLabel ?? t("workspaces:participantPicker.requester")}
            </button>
          )}
          {otherUserOptions
            .filter((option) => {
