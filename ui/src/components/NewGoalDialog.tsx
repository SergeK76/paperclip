      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newGoalDefaults.parentId ? t("newGoalDialog.header.newSubGoal") : t("newGoalDialog.header.newGoal")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("newGoalDialog.fields.titlePlaceholder")}
            value={title}
        ...
        <div className="px-4 pb-2 overflow-y-auto max-h-[50vh]">
          <MarkdownEditor
            ...
            placeholder={t("newGoalDialog.fields.descriptionPlaceholder")}
            bordered={false}
          ...
          <Popover open={levelOpen} onOpenChange={setLevelOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {t(`newGoalDialog.level.${level}`, { defaultValue: level })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-40 p-1" align="start">
              {GOAL_LEVELS.map((l) => (
                ...
                >
                  {t(`newGoalDialog.level.${l}`, { defaultValue: l })}
                </button>
              ))}
            </PopoverContent>
          </Popover>
          ...
          <Popover open={parentOpen} onOpenChange={setParentOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {currentParent ? currentParent.title : t("newGoalDialog.parentGoal.trigger")}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-48 p-1" align="start">
              ...
              >
                {t("newGoalDialog.parentGoal.noParent")}
              </button>
              {(goals ?? []).map((g) => (
                <button
                  key={g.id}
        ...
        <div className="flex items-center justify-end px-4 py-2.5 border-t border-border">
          ...
          >
            {createGoal.isPending
              ? t("newGoalDialog.buttons.creating")
              : newGoalDefaults.parentId
                ? t("newGoalDialog.buttons.createSubGoal")
                : t("newGoalDialog.buttons.createGoal")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
