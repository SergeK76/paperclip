  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {t("agents:panel.heading")}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("agents:panel.noRuns")}</p>
        </div>
      ) : (
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("agents:panel.liveNow") : run.finishedAt ? t("agents:panel.finished", { time: relativeTime(run.finishedAt) }) : t("agents:panel.started", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
