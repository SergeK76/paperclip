        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span>{" "}{t("settings:providerQuota.tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span>{" "}{t("settings:providerQuota.tokensOut")}
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("settings:providerQuota.periodSpend")}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("settings:providerQuota.ofAllocation", { pct: Math.round(budgetPct) })}
              showDeficitNotch={showDeficitNotch}
            />
            <QuotaBar
              label={t("settings:providerQuota.thisWeek")}
              percentUsed={weekPct}
              leftLabel={formatCents(weekSpendCents)}
              rightLabel={t("settings:providerQuota.perWeek", { amount: formatCents(Math.round(weeklyBudgetShare)) })}
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("settings:providerQuota.rollingWindows")}
              </p>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("settings:providerQuota.subscription")}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span>{" "}{t("settings:providerQuota.runs")}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span>{" "}{t("settings:providerQuota.total")}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span>{" "}{t("settings:providerQuota.tokensIn")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span>{" "}{t("settings:providerQuota.tokensOut")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("settings:providerQuota.percentOfSubscription", { pct: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("settings:providerQuota.providerTokenPct", { pct: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("settings:providerQuota.providerCostPct", { pct: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("settings:providerQuota.subscriptionQuota")}
                </p>
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("settings:subscriptionPanel.percentUsed", { pct: qw.usedPercent })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("settings:subscriptionPanel.resetsShort", { date: new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" }) })}
                            </p>
                          ) : null}
                        </div>
                      );
