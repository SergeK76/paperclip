...
export function RunActivityChart({ runs }: { runs: HeartbeatRun[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activity:charts.noRuns")}</p>;
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("activity:charts.noIssues")}</p>;
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`activity:charts.priorityLabels.${p}`, { defaultValue: p.charAt(0).toUpperCase() + p.slice(1) }) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: t(`activity:charts.statusLabels.${s}`, { defaultValue: statusLabels[s] ?? s }) }))} />
    </div>
  );
}
