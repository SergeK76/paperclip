  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <span className="shrink-0 opacity-70">{t("common:worktreeBanner.label")}</span>
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-70" aria-hidden="true" />
        <button
          type="button"
          onClick={handleCopyName}
          title={t("common:worktreeBanner.copyTitle")}
        ...
        >
          {copied ? t("common:worktreeBanner.copied") : branding.name}
        </button>
      </div>
    </div>
  );
