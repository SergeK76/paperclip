  ...
  return (
    <DropdownMenu>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("common:companySwitcher.selectCompany")}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-[220px]">
        <DropdownMenuLabel>{t("common:companySwitcher.companies")}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          <DropdownMenuItem disabled>{t("common:companySwitcher.noCompanies")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild>
          <Link to="/company/settings" className="no-underline text-inherit">
            <Settings className="h-4 w-4 mr-2" />
            {t("common:companySwitcher.companySettings")}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            <Plus className="h-4 w-4 mr-2" />
            {t("common:companySwitcher.manageCompanies")}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
