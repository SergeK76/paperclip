...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("newProjectDialog.errors.localPathInvalid"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("newProjectDialog.errors.repoUrlInvalid"));
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("newProjectDialog.header.title")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("newProjectDialog.fields.namePlaceholder")}
            value={name}
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("newProjectDialog.fields.descriptionPlaceholder")}
            bordered={false}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.fields.repoUrl")}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProjectDialog.fields.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.fields.repoUrlTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProjectDialog.fields.localFolder")}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProjectDialog.fields.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProjectDialog.fields.localFolderTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap">
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("newProjectDialog.fields.goalAdd") : t("newProjectDialog.fields.goal")}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              {selectedGoals.length === 0 && (
                ...
                >
                  {t("newProjectDialog.fields.noGoal")}
                </button>
              )}
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t("newProjectDialog.fields.allGoalsSelected")}
                </div>
              )}
            </PopoverContent>
          </Popover>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs">
            ...
            <input
              ...
              placeholder={t("projects:newProjectDialog.fields.targetDate")}
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          ...
          >
            {createProject.isPending ? t("newProjectDialog.buttons.creating") : t("newProjectDialog.buttons.create")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
