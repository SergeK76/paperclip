    ...
    <>
      <div className={cn(
        ...
        >
          ...
          {t("common:outputFeedback.helpful")}
        </Button>
        <Button
          type="button"
          size="sm"
        ...
        >
          ...
          {t("common:outputFeedback.needsWork")}
        </Button>
        {rightSlot ? <div className="ml-auto">{rightSlot}</div> : null}
      </div>
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("common:outputFeedback.whatCouldBeBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("common:outputFeedback.addNote")}
          ...
          <div className="mt-3 flex items-center justify-end gap-2">
            ...
            >
              {t("common:outputFeedback.dismiss")}
            </Button>
            <Button
              type="button"
              size="sm"
            ...
            >
              {isSaving ? t("common:outputFeedback.saving") : t("common:outputFeedback.saveNote")}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("common:outputFeedback.dialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("common:outputFeedback.dialogDesc")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("common:outputFeedback.alwaysSavedLocally")}
            </p>
            <p>
              {t("common:outputFeedback.alwaysAllowDesc")}
            </p>
            <p>
              {t("common:outputFeedback.changeInSettings")}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
              ...
              >
                {t("common:outputFeedback.readTerms")}
              </a>
            ) : null}
          </div>
          <DialogFooter>
            ...
            >
              {isSaving ? t("common:outputFeedback.saving") : t("common:outputFeedback.dontAllow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingVote || isSaving}
            ...
            >
              {isSaving ? t("common:outputFeedback.saving") : t("common:outputFeedback.alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
