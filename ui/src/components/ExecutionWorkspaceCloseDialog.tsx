...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? t("workspaces:closeDialog.actionRetry") : t("workspaces:closeDialog.actionClose");
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("workspaces:closeDialog.toast.retried") : t("workspaces:closeDialog.toast.closed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("workspaces:closeDialog.toast.closeFailed"),
        body: error instanceof Error ? error.message : t("workspaces:closeDialog.toast.unknownError"),
        tone: "error",
      });
    },
  });
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("workspaces:closeDialog.checkingReadiness")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("workspaces:closeDialog.readinessError")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("workspaces:closeDialog.state.blocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("workspaces:closeDialog.state.ready_with_warnings")
                    : t("workspaces:closeDialog.state.ready")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("workspaces:closeDialog.workspaceDesc.shared")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("workspaces:closeDialog.workspaceDesc.ownCheckout")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("workspaces:closeDialog.workspaceDesc.primaryPath")
                      : t("workspaces:closeDialog.workspaceDesc.disposable")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.blockingIssues")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("workspaces:closeDialog.git.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("workspaces:closeDialog.git.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("workspaces:closeDialog.git.unknown") : readiness.git.isMergedIntoBase ? t("workspaces:closeDialog.git.yes") : t("workspaces:closeDialog.git.no")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.dirtyFiles")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("workspaces:closeDialog.git.untrackedFiles")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.otherIssues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.runtimeServices")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("workspaces:closeDialog.noDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("workspaces:closeDialog.sections.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("workspaces:closeDialog.retryNote")}
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
                {t("workspaces:closeDialog.alreadyArchived")}
              </div>
            ) : null}
