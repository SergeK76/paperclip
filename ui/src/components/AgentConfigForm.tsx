    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("agents:config.saving") : t("common:actions.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents:config.sections.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents:config.sections.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agents:config.fields.name")} hint={help.name}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("agents:config.placeholders.agentName")}
              />
            </Field>
            <Field label={t("agents:config.fields.title")} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("agents:config.placeholders.titleExample")}
              />
            </Field>
            <Field label={t("agents:config.fields.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("agents:config.placeholders.chooseManager")}
              />
            </Field>
            <Field label={t("agents:config.fields.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agents:config.placeholders.capabilities")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agents:config.fields.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("agents:config.placeholders.promptTemplate")}
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agents:config.sections.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agents:config.sections.adapter")}</span>
          }
          {showAdapterTestEnvironmentButton && (
            ...
            >
              {testEnvironment.isPending ? t("agents:config.testing") : t("agents:config.testEnvironment")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agents:config.fields.adapterType")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
                disabledTypes={disabledTypes}
                onChange={(newAdapterType) => {
                  if (isCreate) {
                    // Reset all adapter-specific fields to defaults when switching adapter type
                    const { adapterType: _at, ...defaults } = defaultCreateValues;
                    const nextValues: CreateConfigValues = { ...defaults, adapterType: newAdapterType };
                    if (newAdapterType === "codex_local") {
                      nextValues.model = DEFAULT_CODEX_LOCAL_MODEL;
                      nextValues.dangerouslyBypassSandbox =
                        DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX;
                    } else if (newAdapterType === "gemini_local") {
                      nextValues.model = DEFAULT_GEMINI_LOCAL_MODEL;
                    } else if (newAdapterType === "cursor") {
                      nextValues.model = DEFAULT_CURSOR_LOCAL_MODEL;
                    } else if (newAdapterType === "opencode_local") {
                      nextValues.model = "";
                    }
                    set!(nextValues);
                  } else {
                    ...
                    setOverlay((prev) => ({
                      ...
                      adapterConfig: {
                        model:
                          newAdapterType === "codex_local"
                            ? DEFAULT_CODEX_LOCAL_MODEL
                            : newAdapterType === "gemini_local"
                              ? DEFAULT_GEMINI_LOCAL_MODEL
                            : newAdapterType === "cursor"
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agents:config.fields.workingDirDeprecated")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents:config.sections.permissions")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents:config.sections.permissions")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("agents:config.fields.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agents:config.fields.bootstrapPrompt")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agents:config.placeholders.bootstrapPrompt")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agents:config.hints.bootstrapLegacy")}
                  </div>
                </>
              )}
              ...
              <Field label={t("agents:config.fields.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("agents:config.placeholders.extraArgs")}
                />
              </Field>

              <Field label={t("agents:config.fields.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agents:config.fields.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agents:config.fields.graceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agents:config.sections.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agents:config.sections.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agents:config.fields.heartbeatOnInterval")}
              ...
              numberLabel="sec"
              numberPrefix={t("agents:config.fields.runHeartbeatEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("agents:config.sections.advancedRunPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agents:config.fields.wakeOnDemand")}
              ...
              />
              <Field label={t("agents:config.fields.cooldownSec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agents:config.fields.maxConcurrentRuns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass" ? t("agents:config.envPassed") : result.status === "warn" ? t("agents:config.envWarnings") : t("agents:config.envFailed");
  const statusClass =
    result.status === "pass"
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agents:config.fields.model")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("agents:config.modelManual")}</span>
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("agents:config.fields.thinkingEffort")} hint={help.thinkingEffort}>
