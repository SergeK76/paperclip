...
function resolveActorLabel(
  ...
  if (actorType === "agent" && actorId) {
    return resolveAgentName(queryClient, companyId, actorId) ?? t("common:liveUpdates.actors.agentFallback", { id: shortId(actorId) });
  }
  if (actorType === "system") return t("common:liveUpdates.actors.system");
  if (actorType === "user" && actorId) {
    return resolveUserName(queryClient, companyId, actorId) ?? t("common:liveUpdates.actors.board");
  }
  return t("common:liveUpdates.actors.someone");
}
...
function describeIssueUpdate(details: Record<string, unknown> | null, t: TFunction): string | null {
  if (!details) return null;
  const changes: string[] = [];
  if (typeof details.status === "string") changes.push(t("common:liveUpdates.issue.statusChanged", { status: details.status.replace(/_/g, " ") }));
  if (typeof details.priority === "string") changes.push(t("common:liveUpdates.issue.priorityChanged", { priority: details.priority }));
  if (typeof details.assigneeAgentId === "string" || typeof details.assigneeUserId === "string") {
    changes.push(t("common:liveUpdates.issue.reassigned"));
  } else if (details.assigneeAgentId === null || details.assigneeUserId === null) {
    changes.push(t("common:liveUpdates.issue.unassigned"));
  }
  if (details.reopened === true) {
    const from = readString(details.reopenedFrom);
    changes.push(from ? t("common:liveUpdates.issue.reopenedFrom", { from: from.replace(/_/g, " ") }) : t("common:liveUpdates.issue.reopened"));
  }
  if (typeof details.title === "string") changes.push(t("common:liveUpdates.issue.titleChanged"));
  if (typeof details.description === "string") changes.push(t("common:liveUpdates.issue.descriptionChanged"));
...
function buildActivityToast(
  ...
  if (action === "issue.created") {
    return {
      title: t("common:liveUpdates.issue.created", { actor, ref: issue.ref }),
      body: issue.title ? truncate(issue.title, 96) : undefined,
      tone: "success",
      action: { label: t("common:liveUpdates.viewRef", { ref: issue.ref }), href: issue.href },
  ...
  if (action === "issue.updated") {
    ...
    const body = changeDesc
      ...
      : issue.title
        ...
        : issue.label;
    return {
      title: t("common:liveUpdates.issue.updated", { actor, ref: issue.ref }),
  ...
  const reopenedLabel = reopened
    ? reopenedFrom
      ? t("common:liveUpdates.issue.reopenedFrom", { from: reopenedFrom.replace(/_/g, " ") })
      : t("common:liveUpdates.issue.reopened")
    : null;
  const title = reopened
    ? t("common:liveUpdates.issue.reopenedAndCommented", { actor, ref: issue.ref })
    : updated
      ? t("common:liveUpdates.issue.commentedAndUpdated", { actor, ref: issue.ref })
      : t("common:liveUpdates.issue.commented", { actor, ref: issue.ref });
...
function buildJoinRequestToast(
  ...
  const label = requestType === "agent" ? t("common:liveUpdates.actors.agentType") : t("common:liveUpdates.actors.someone");

  return {
    title: t("common:liveUpdates.joinRequest.wantsToJoin", { label }),
    body: t("common:liveUpdates.joinRequest.body"),
    tone: "info",
    action: { label: t("common:liveUpdates.viewInbox"), href: "/inbox/mine" },
...
function buildAgentStatusToast(
  ...
  const name = nameOf(agentId) ?? t("common:liveUpdates.actors.agentFallback", { id: shortId(agentId) });
  const title =
    status === "running"
      ? t("common:liveUpdates.agentStatus.started", { name })
      : t("common:liveUpdates.agentStatus.errored", { name });
  ...
  return {
    ...
    body,
    tone,
    action: { label: t("common:liveUpdates.viewAgent"), href: `/agents/${agentId}` },
...
function buildRunStatusToast(
  ...
  const statusLabel =
    status === "succeeded" ? t("common:liveUpdates.runStatus.succeeded")
      : status === "failed" ? t("common:liveUpdates.runStatus.failed")
        : status === "timed_out" ? t("common:liveUpdates.runStatus.timedOut")
          : t("common:liveUpdates.runStatus.cancelled");
  const title = t("common:liveUpdates.runStatus.title", { name, statusLabel });

  let body: string | undefined;
  if (error) {
    body = truncate(error, 100);
  } else if (triggerDetail) {
    body = t("common:liveUpdates.runStatus.trigger", { detail: triggerDetail });
  }
  ...
  return {
    ...
    action: { label: t("common:liveUpdates.viewRun"), href: `/agents/${agentId}/runs/${runId}` },
